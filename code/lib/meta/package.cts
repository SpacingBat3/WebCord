/*
 * package.ts – Scripts, types and typeguards for loading `package.json` file in
 *              runtime.
 */
 ...
 * 
 * The class has only support for limited properties — this class is designed to
 * gather only some metadata about the packages for representative purposes,
 * like `version`,`author`,`homepage` or `dependencies`.
 */
export class PackageJSON<T extends (keyof PackageJsonProperties)[]> {
  readonly #regexp = Object.freeze({
  ...
  private checkPackageJsonComplete(object: unknown): string {
    ...
    if((this.#regexp.name.exec((object as PackageJsonProperties).name)) === null)
    ...
    if(!this.#regexp.license.test((object as PackageJsonProperties).license))
      try {
